...
class SerialCommandDriver:
    ...
    def finalize_log(self):
        ...
        if self.error_log:
            for error in self.error_log:
                self.log(f"Error at cycle {error['cycle']}: {error['error']}")
        if self.log_file and not self.log_file.closed:
            self.log_file.close()
    ...
    def receive_response(self, timeout_seconds):
        if self.ser and self.ser.is_open:
            start_time = time.time()
            while time.time() - start_time < timeout_seconds:
                response = self.ser.readline().decode('utf-8').strip()
                sanitized = self.sanitize_response(response)
                if not sanitized or sanitized == "NO" or sanitized == "NOO":
                    continue
...
class SerialGUI:
    def __init__(self, root):
        ...
        try:
            icon = PhotoImage(file="logo.png")  # Replace with your actual icon path
 ...
 # Start button
        ...
        # Add this in SerialGUI.__init__ after self.num_cycles_entry.pack():
        self.delay_label = tk.Label(root, text="Delay between cycles (seconds):")
        self.delay_label.pack(pady=(0, 0))

        self.delay_entry = tk.Entry(root)
        self.delay_entry.insert(0, "1")  # Default to 1 second
    ...
    def wait_for_decap(self):
        while True:
            response = self.driver.receive_response(timeout_seconds=300)
            if response == "STATUS_WRONG_TUBE":
                self.driver.log("STATUS_WRONG_TUBE received. Retrying DECAP...")
                self.driver.send_command("B\n")
                continue
            elif response == "BOK":
                self.driver.log("Received 'BOK'. Waiting for DECAP_OK or DECAP_ERR...")
                # After receiving BOK, continue loop to wait for final response
                continue
            elif response in ["DECAP_OK", "DECAP_ERR"]:
                return response
            else:
                return None
        ...
        while not self.stop_event.is_set():
            ...
            if self.fixed_cycles_var.get():
                try:
                    ...
                    self.driver.log("Invalid number of cycles.")
                    break
            ...
            if response == "BOK":
                self.driver.log("Received 'BOK'. Now waiting for DECAP response...")
                decap_result = self.wait_for_decap()

                if decap_result == "DECAP_OK":
                    self.driver.log("Received 'DECAP_OK'. Now sending 'C'...")
                    self.driver.send_command("C\n")
                    cok_response = self.driver.receive_response(timeout_seconds=60)

                    if cok_response == "COK":
                        recap_result = self.wait_for_recap()

                        if recap_result == "RECAP_OK":
                            self.driver.log("Received 'RECAP_OK'. Process complete.")
                        elif recap_result == "RECAP_ERR":
                            self.driver.error_log.append({
                                'cycle': self.driver.cycle_counter,
                                'error': 'RECAP_ERR'
                            })
                            self.driver.log("RECAP_ERR received. Stopping.")
                            break
                        else:
                            self.driver.error_log.append({
                                'cycle': self.driver.cycle_counter,
                                'error': f"Unexpected recap response: {recap_result}"
                            })
                            self.driver.log("Did not receive 'RECAP_OK'. Stopping.")
                            break
                    else:
                        self.driver.error_log.append({
                            'cycle': self.driver.cycle_counter,
                            'error': "Did not receive 'COK'"
                        })
                        self.driver.log("Did not receive 'COK'. Stopping.")
                        break

                elif decap_result == "DECAP_ERR":
                    self.driver.error_log.append({
                        'cycle': self.driver.cycle_counter,
                        'error': 'DECAP_ERR'
                    })
                    self.driver.log("DECAP_ERR received. Stopping.")
                    break
                else:
                    self.driver.error_log.append({
                        'cycle': self.driver.cycle_counter,
                        'error': f"Unexpected decap response: {decap_result}"
                    })
                    self.driver.log("Did not receive 'DECAP_OK'. Stopping.")
                    break
            else:
                self.driver.error_log.append({
                    'cycle': self.driver.cycle_counter,
                    'error': f"Did not receive 'BOK'. Got: {response}"
                })
                self.driver.log("Did not receive 'BOK'. Stopping.")
                break

            try:
                delay = float(self.delay_entry.get())
            except ValueError:
                delay = 1  # Default fallback if user enters invalid input
            self.driver.log(f"Waiting for {delay} seconds before next cycle...")
            time.sleep(delay)
